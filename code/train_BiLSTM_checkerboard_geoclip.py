...
if __name__ == "__main__":
    ...
    X_emb = np.load(f'../data/X_{year}_geoclip.npy')
    ...
    # Only keep non-negative records
    X = X[(y > 0).squeeze(), :, :][:,:,mask]
    X_emb = X_emb[(y > 0).squeeze(), :][:, 2:]
    fold_idx = fold_idx[(y > 0).squeeze()]
    y = y[(y > 0).squeeze(), :]


    train_size = X.shape[0]
    emb_size = X_emb.shape[0]
    time_lag = X.shape[1]
    n_features = X.shape[2]

    print(f"Total Training Samples: {X.shape}, {X_emb.shape},{y.shape}")
    ...
    for fold_num in n_folds:#range(10):
        ...
        train_X = X[train_idx]
        train_X_emb = X_emb[train_idx]
        train_y = y[train_idx]
        test_X = X[test_idx]
        test_X_emb = X_emb[test_idx]
        test_y = y[test_idx]
        ...
        if fold_num in []:
            ...
            # Save the model
            model.save(f"../model/LSTM_Model/checkerboard_fold2_deg16/geoclip_hadamard/LSTM_{fold_num}.h5")
            # Save the model in the TensorFlow SavedModel format
            # model_save_path = f"../model/LSTM_Model/LSTM_{fold_num}"
