...
if __name__ == "__main__":
    ...
    lat = np.deg2rad(CV_X[:, :, 5])  # convert latitude to radians
    lon = np.deg2rad(CV_X[:, :, 6])  # convert longitude to radians
    ...
    # Apply Sinusoidal Encoding
    lat = np.deg2rad(test_X[:, :, 5])  # convert latitude to radians
    lon = np.deg2rad(test_X[:, :, 6])  # convert longitude to radians
    sin_lat, cos_lat = np.sin(lat), np.cos(lat)
    sin_lon, cos_lon = np.sin(lon), np.cos(lon)
