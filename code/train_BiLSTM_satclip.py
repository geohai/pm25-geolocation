...
def fit_lstm_model(train_X, train_X_emb, train_y,
    ...
    loc_input = Input(shape=(256,), name='location_input')
    loc_proj = LayerNormalization()(loc_input)
    loc_proj = Dense(256, activation='tanh', name='loc_proj')(loc_proj)
    ...
    #     initial_learning_rate=1e-2, decay_steps=50000, decay_rate=0.85, staircase=True
    # )
    ...
    es = tf.keras.callbacks.EarlyStopping(monitor='val_loss', mode='min', patience=90, verbose=1)
...
if __name__ == "__main__":
    ...
    CV_X_emb = np.load(f'../data/X_{year}_CV_satclip.npy')[:, 2:]
    ...
    test_X_emb = np.load('../data/X_2021_test_satclip.npy')[:, 2:]
    ...
    for train_idx, val_idx in kfold.split(X):
        ...
        if fold_num in []:
            ...
            # Evaluate validation
            print("============= Validation =================")
            pred_val = make_predictions(model=model, n_batch=2 ** 8, inputs=[val_X, val_X_emb])
            ...
            # Evaluate test
            print("============= Test =================")
            pred_test = make_predictions(model=model, n_batch=2 ** 8, inputs=[test_X, test_X_emb])
